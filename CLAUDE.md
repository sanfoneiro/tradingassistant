...
| Grouped daily vs per-ticker bars | every OHLCV field, before substituting the cheap source for the expensive one | `src/db/backfill-bars.ts` |

That third row is the newest and the pattern is worth copying. Grouped daily
returns 12,485 tickers in ONE call where the per-symbol endpoint costs one call
each — but before trusting it, both were fetched for the same session and
compared field by field. `o h l c v vw n` are identical; **`t` differs by
sixteen hours**, because grouped stamps the close (20:00 UTC) and per-ticker
stamps the session start (midnight ET). The `bars` table therefore stores a
DATE, which makes that difference unrepresentable rather than merely handled.
...
**Except that on this data plan there is no forming bar.** Stocks are
previous-day-only: a request for today returns `403 NOT_AUTHORIZED`, while a
*range* through today returns 200 with the series silently truncated at the
prior close. So `--after-open` was pricing zones against yesterday AND, via
`slice(0, -1)`, discarding a completed bar it mistook for a forming one —
leaving trend classification two sessions behind while the run reported clean.
It now asks `barState()` what session the last bar covers (`src/lib/session.ts`,
...
- **The `vercel` CLI IS authenticated** (`whoami` -> `oron-3812`, scope `achievement`).
  Verified 2026-08-27; the older note here said it was not. `vercel cron ls`,
  `metrics`, `usage` and `contract` all answer. `vercel api` refuses in
  non-interactive agent mode, so read data through the named subcommands.
...
- **The stocks plan is PREVIOUS-DAY-ONLY, and it fails two different ways.**
  A direct request for today is `403 NOT_AUTHORIZED`; a *range* through today
  is `200` with the series quietly truncated. Anything reasoning about "now"
  must ask what session the newest bar covers rather than assume it is today —
  `barState()` in `src/lib/session.ts`. Futures behave differently again:
  session bars arrive same-day, 1-minute bars run 8 hours late, and real-time
  snapshots are 403 on both asset classes.
- **Massive is a Polygon surface.** Error bodies name `polygon.io` and the SMA
  response leaks `api.polygon.io` in `underlying.url`. Polygon's docs are the
  better reference when Massive's are thin.
...
src/lib/session.ts     what session a bar covers, and whether it is forming;
                       ET wall-clock -> UTC (Finviz publishes naive ET)
...
- **MAE/MFE aside, the calendar now exists** (`npm run calendars:sync`, 2,371
  ...
  **An empty Finviz day means UNKNOWN, never "nothing scheduled":** 2026-10-05
  returns 0 while 2026-10-19 returns 12 and 2026-11-16 returns 109, and a real
  market holiday returns 0 too. Beyond ~2 weeks every earnings date carries
